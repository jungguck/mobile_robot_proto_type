...
- `docs/DEBUG_LOG_2026-09-22.md`: **IMU 를 선택 사항으로 내림** (`use_imu` 스위치).
  `odom->base_link` TF 발행 책임을 드라이버/EKF 사이에서 전환, `/odom_raw` 구독자 전수 정리,
  매핑 속도 제약(각속도 0.5 rad/s) 근거. **저장소 시차 사고** — 2주 낡은 트리 위에서 작업.
  저장소 정리(`old_file/` 신설, 활성 파일 26개), `relayrobot_driver` 폐지(패키지 6→5),
  **README 두 벌을 하나로 통합**.
  ⏳ **젯슨 빌드 미실시** (전원 꺼짐).
